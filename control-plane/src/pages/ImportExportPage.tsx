...
function checkRow(id: string, label: string, checked: boolean, meta: string, onToggle: () => void) {
  return (
    <label
      key={id}
      style={{
...
export function ImportExportPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setCat(next);
      setErr(catalogHasSecrets(next) ? t("impexp.leak") : "");
    } catch (e) {
  ...
  async function onFile(file: File | undefined) {
    ...
    try {
      ...
      if (publicHasSecrets(prev)) {
        ...
        setPreview(prev);
        setImportJob(null);
        setErr(prev.valid ? "" : prev.errors.join("; ") || t("impexp.invalid"));
  ...
  async function runImport(dry: boolean) {
    if (!preview?.archive) {
      setErr(t("impexp.needFile"));
      return;
    }
    if (!preview.valid) {
      setErr(t("impexp.invalid"));
      return;
    }
    if (!dry && !confirmImport) {
      setErr(t("impexp.needConfirm"));
      return;
    }
  ...
  async function runRollback() {
    if (!importJob?.id || importJob.dry_run) return;
    setBusy("rollback");
    try {
  ...
  return (
    <div style={{ padding: "14px 22px 40px", display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {tab === "teams" ? (
        <Card>
          <CardHeader icon="layers" title={t("impexp.tab.teams")} meta={t("impexp.meta", { n: cat.teams.length })} />
          {!loading && cat.teams.length === 0 ? <EmptyState>{t("impexp.empty.teams")}</EmptyState> : null}
          {cat.teams.map((row) =>
            checkRow(row.id, row.name, sel.team_ids.includes(row.id), row.id, () => setSel((s) => ({ ...s, team_ids: toggleId(s.team_ids, row.id) }))),
          )}
        </Card>
      ) : null}
      ...
      {tab === "agents" ? (
        <Card>
          <CardHeader icon="bolt" title={t("impexp.tab.agents")} meta={t("impexp.meta", { n: cat.agents.length })} />
          {!loading && cat.agents.length === 0 ? <EmptyState>{t("impexp.empty.agents")}</EmptyState> : null}
          {cat.agents.map((row) =>
            checkRow(
              row.id,
      ...
      {tab === "skills" ? (
        <>
          <Card>
            <CardHeader icon="build" title={t("impexp.skills")} meta={t("impexp.meta", { n: cat.skills.length })} />
            {!cat.skills_configured ? <p style={{ margin: 0, padding: "0 16px 10px", fontSize: 12.5, color: "var(--text-3)" }}>{t("impexp.skillsOff")}</p> : null}
            {!loading && cat.skills.length === 0 ? <EmptyState>{t("impexp.empty.skills")}</EmptyState> : null}
            {cat.skills.map((row) =>
              checkRow(row.name, row.name, sel.skill_names.includes(row.name), row.path || "", () =>
                setSel((s) => ({ ...s, skill_names: toggleId(s.skill_names, row.name) })),
              ),
            )}
          </Card>
          <Card>
            <CardHeader icon="hook" title={t("impexp.mcp")} meta={t("impexp.meta", { n: cat.mcp.length })} />
            {!loading && cat.mcp.length === 0 ? <EmptyState>{t("impexp.empty.mcp")}</EmptyState> : null}
            {cat.mcp.map((row) =>
              checkRow(
                row.name,
                row.name,
        ...
        <>
          <Card>
            ...
            <div style={{ padding: "0 16px 16px", display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              {!selected && !loading ? <EmptyState>{t("impexp.empty.select")}</EmptyState> : null}
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <Button variant="accent" disabled={!selected || Boolean(busy)} onClick={() => void runExport()}>
        ...
        <>
          <Card>
            ...
            <div style={{ padding: "0 16px 16px", display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <Button disabled={!preview?.valid || Boolean(busy)} onClick={() => void runImport(true)}>
                  {busy === "dry" ? t("impexp.running") : t("impexp.dry")}
                </Button>
                <Button variant="accent" disabled={!preview?.valid || Boolean(busy) || !confirmImport} onClick={() => void runImport(false)}>
                  {busy === "import" ? t("impexp.importing") : t("impexp.import")}
                </Button>
                {importJob && !importJob.dry_run && importJob.kind === "import" && importJob.status === "done" ? (
                  <Button disabled={Boolean(busy)} onClick={() => void runRollback()}>
                    {t("impexp.rollback")}
                  </Button>
                ) : null}
              </div>
