...
Environment:
  ...
  GOSO_VIEW_TOKEN          Optional GET-only token for /healthz /api/agents /api/sessions /api/nodes /api/workstations /api/storage /api/events /api/activity /api/logs /api/tenant /api/tenants /api/api-keys /api/packages /api/approvals
  ...
  API Keys                 GET /api/api-keys list/detail (prefix+hash metadata, never secret). POST create reveals secret once. POST revoke with confirm. Audit create/revoke without the secret.
