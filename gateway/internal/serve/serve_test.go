...
func TestViewToken_GETOnly(t *testing.T) {
	...
	h.ServeHTTP(rr, req)
	if rr.Code != http.StatusOK {
		t.Fatalf("view GET logs %d %s", rr.Code, rr.Body.String())
	}
	...
	h.ServeHTTP(rr, req)
	if rr.Code != http.StatusForbidden {
		t.Fatalf("view POST logs %d %s", rr.Code, rr.Body.String())
	}
	...
	h.ServeHTTP(rr, req)
	if rr.Code != http.StatusOK {
		t.Fatalf("view GET tenants %d %s", rr.Code, rr.Body.String())
	}
	...
	h.ServeHTTP(rr, req)
	if rr.Code != http.StatusOK {
		t.Fatalf("view GET packages %d %s", rr.Code, rr.Body.String())
	}

	req = httptest.NewRequest(http.MethodPost, "/api/packages/install", strings.NewReader("{}"))
	req.Header.Set("Authorization", "Bearer view-041")
	req.Header.Set("Content-Type", "application/json")
	rr = httptest.NewRecorder()
	h.ServeHTTP(rr, req)
	if rr.Code != http.StatusForbidden {
		t.Fatalf("view POST packages %d %s", rr.Code, rr.Body.String())
	}
	...
	h.ServeHTTP(rr, req)
	if rr.Code != http.StatusOK {
		t.Fatalf("view GET approvals %d %s", rr.Code, rr.Body.String())
	}

	req = httptest.NewRequest(http.MethodPost, "/api/approvals/x/decision", strings.NewReader("{}"))
	req.Header.Set("Authorization", "Bearer view-041")
	req.Header.Set("Content-Type", "application/json")
	rr = httptest.NewRecorder()
	h.ServeHTTP(rr, req)
	if rr.Code != http.StatusForbidden {
		t.Fatalf("view POST approvals %d %s", rr.Code, rr.Body.String())
	}

	req = httptest.NewRequest(http.MethodGet, "/api/import-export", nil)
	req.Header.Set("Authorization", "Bearer view-041")
	rr = httptest.NewRecorder()
	h.ServeHTTP(rr, req)
	if rr.Code != http.StatusOK {
		t.Fatalf("view GET import-export %d %s", rr.Code, rr.Body.String())
	}

	req = httptest.NewRequest(http.MethodPost, "/api/import-export/export", strings.NewReader("{}"))
	req.Header.Set("Authorization", "Bearer view-041")
	req.Header.Set("Content-Type", "application/json")
	rr = httptest.NewRecorder()
	h.ServeHTTP(rr, req)
	if rr.Code != http.StatusForbidden {
		t.Fatalf("view POST import-export %d %s", rr.Code, rr.Body.String())
	}
	...
	h.ServeHTTP(rr, req)
	if rr.Code != http.StatusOK {
		t.Fatalf("view GET tts %d %s", rr.Code, rr.Body.String())
	}

	req = httptest.NewRequest(http.MethodPut, "/api/tts", strings.NewReader("{}"))
	req.Header.Set("Authorization", "Bearer view-041")
	req.Header.Set("Content-Type", "application/json")
	rr = httptest.NewRecorder()
	h.ServeHTTP(rr, req)
	if rr.Code != http.StatusForbidden {
		t.Fatalf("view PUT tts %d %s", rr.Code, rr.Body.String())
	}

	req = httptest.NewRequest(http.MethodPost, "/api/tts/test", strings.NewReader("{}"))
	req.Header.Set("Authorization", "Bearer view-041")
	req.Header.Set("Content-Type", "application/json")
	rr = httptest.NewRecorder()
	h.ServeHTTP(rr, req)
	if rr.Code != http.StatusForbidden {
		t.Fatalf("view POST tts test %d %s", rr.Code, rr.Body.String())
	}
