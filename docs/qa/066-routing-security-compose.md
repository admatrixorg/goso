...
- SSRF DNS-aware when `GOSO_SSRF=1`: `LookupIP`; lookup fail denies; any loopback/private/link-local/unspecified/multicast / IPv6 unique-local / `169.254.169.254` denies. `CheckURL` on LLM probe + OpenAI/Anthropic chat; `GuardClient` on those HTTP clients. `GOSO_SSRF=0` still allows demo `http://127.0.0.1:20127/v1`.
