...
| GOSO_TTS_PROVIDER / GOSO_TTS_API_KEY / GOSO_TTS_VOICE / GOSO_TTS_MODEL / GOSO_TTS_ENDPOINT / GOSO_TTS_REGION / GOSO_TTS_LANGUAGE / GOSO_TTS_AUTO_APPLY / GOSO_TTS_MAX_CHARS / GOSO_TTS_TIMEOUT_MS / GOSO_TTS_ENABLED | (empty) | SPEC 118 TTS operator config overlay. `GET /api/tts` returns `key_set`/`configured` only — never the key. Env-owned key refuses PUT secrets (409). Builtin `tts` invoke stays fail-closed without a test double (see `GOSO_MEDIA`). |
